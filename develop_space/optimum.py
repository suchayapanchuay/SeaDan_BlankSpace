...
def create_gui():
    ...
    result_label.grid(row=10, column=0, columnspan=2, pady=20, padx=20)

    man_btn = tk.Button(root, text="Help", command=show_man,width=8, height=2)
    man_btn.grid(row=11, column=0 ,columnspan=2, pady=10, padx=20, sticky="n")
    
    text_frame = tk.Label(root, text="After\n(ข้อมูลหลังจากการเปลี่ยนแปลง)\n\nBefore\n(ข้อมูลก่อนการเปลี่ยนแปลง)\n\nAfter - Before = Volumn Change", font=("Helvetica", 12,"bold"), bg="#F0C38E", fg="#0F2573")
    ...
    # Delta altitude matrix and volume change calculation
        ...
        result_text.config(state=tk.DISABLED)
        copy_label = tk.Button(root, text="Copy", command=copy_to_clipboard, fg="blue", cursor="hand2", font=("Arial", 12, "underline"))
    ...
    canvas = tk.Canvas(root, width=70, height=90, bg='#F0C38E')
    
    dolphin_gif = Image.open('/Users/suchayapanchuay/Documents/Project/blankspace/image/gif2.gif')  # ใส่พาธ GIF ของคุณ
    dolphin_sequence = [
        ImageTk.PhotoImage(img.resize((70, 90), Image.Resampling.LANCZOS))
        for img in ImageSequence.Iterator(dolphin_gif)
    ]
    dolphin_id = canvas.create_image(35, 45, anchor="center", image=dolphin_sequence[0])
    canvas.grid(row=10,column=1, rowspan=2, padx=(10,5), pady=10, sticky="n")
