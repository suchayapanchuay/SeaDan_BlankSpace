...
def create_gui():
    ...
    def user_manual():
              man_window = tk.Toplevel(root) 
              man_window.title("User Manual")
              man_window.geometry("600x400")
              text = """
                    User Manual:
                    1. กดปุ่ม Start เพื่อเริ่มใช้งาน
                    2. กดปุ่ม Stop เพื่อหยุดการทำงาน
                    3. กดปุ่ม Exit เพื่อออกจากโปรแกรม
                    """
              label = tk.Label(root, text=text, justify="left", padx=10, pady=10, anchor="w")
